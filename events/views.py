...
import time
from scripts.helpful_scripts import event_to_series
...
class RetrieveEvents(APIView):
    def get(self, request, slug, format=None):
        if slug:
            ...
            try:
                data_from_db = Event.objects.filter(collection_slug=slug).order_by(
                    "-timestamp"
                )
            except:
                data_from_db = None

            os_api_key = os.getenv("X_API_KEY")

            if data_from_db is not None and len(data_from_db) > 0:
                data += data_from_db
                latest_time = (
                    data_from_db.first().timestamp
                )  # ??? to feed into occurred_after?
            i = 0
            while True:
                querystring = {
                ...
                headers = {
                    ...
                    "X-API-KEY": "bd4f5e03dd8847a8bef273d87801616a",
                }
                print(querystring)
                    ...
                    for trade in json:
                        ...
                        try:
                            e = Event.objects.create(
                                ...
                                contract_address=trade["asset"]["asset_contract"][
                                    "address"
                                ],
            ...
            if len(data) <= 1000:
                ...
                compressed_data = [
                    data[i // denominator_length]
                    for i in range(
                        0, numerator_length * MAX_POINTS_TO_RENDER, numerator_length
                    )
                ]
            serializer = EventSerializer(compressed_data, many=True)
            resp_data = event_to_series(serializer.data)
